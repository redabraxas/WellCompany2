...
public class MyApplyFragment extends Fragment {
    ...
    ArrayList<Reserve> reserveApplyList = new ArrayList<Reserve>();
...
//
//         m_cWidth = (   ViewGroup.LayoutParams.MATCH_PARENT - ( m_lineSize * COLS - 1 ) ) / COLS ;
//        m_cHeight = (  ViewGroup.LayoutParams.MATCH_PARENT - ( m_lineSize * ROWS - 1 ) ) / ROWS ;
//        m_tcHeight = (  ViewGroup.LayoutParams.MATCH_PARENT - ( m_lineSize * COLS - 1 ) ) / COLS ;
    }
...
/// 토요일과 일요일은 다른 컬러로 표시한다.
    ...
    public void myClickEvent( String code, final int year, final int month, final int day )
    {
        ...
        waitApplyList.clear();
        approveApplyList.clear();
        rejectApplyList.clear();

        if(code.equals("sure") || code.equals("wait")){
            // 이 날짜의 예약 리스트 검사
            for(int i=0; i< reserveApplyList.size(); i++){
                if(reserveApplyList.get(i).getDay() == day){
                    switch (reserveApplyList.get(i).getState()) {
                        case 0:
                            waitApplyList.add(reserveApplyList.get(i));
                            break;
                        case 1:
                            approveApplyList.add(reserveApplyList.get(i));
                            break;
                        case 2:
                            rejectApplyList.add(reserveApplyList.get(i));
                            break;
